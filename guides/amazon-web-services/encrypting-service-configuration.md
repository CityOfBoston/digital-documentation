...
Apps are configured by putting files in a secure configuration bucket on S3. The `entrypoint.sh`script for our apps pulls all files in from the app’s path in the bucket before starting up. This allows an app to be securely configured with a `.env` file and, for example, `server.crt` and `server.key` files for TLS connections.
...
1. Look at the Outputs of the “CommonResources” CloudFormation stack for the service for its **Config Key Alias** value. Save it in the `$CONFIG_KEY_ALIAS` environment variable.
2. Log in to AWS CLI with an account that has `KMS::encrypt` permissions for the key.
3. Run the following command **with a leading space so that it doesn’t appear in your command history**: `aws kms encrypt --output text --key-id $CONFIG_KEY_ALIAS --plaintext …`
4. Copy the encrypted value \(the output up to the whitespace\) as the value in `.env`: `PASSWORD_KMS_ENCRYPTED=…`
